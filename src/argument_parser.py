...
def get_argument_parser():
    ...
    parser.add_argument(
        "--width",
        dest="width",
        help="width in px, the frame shall be resized to (has to equal '--height', if '--square' is set)",
        default=None,
        type=int,
    )
    parser.add_argument(
        "--height",
        dest="height",
        help="height in px, the frame shall be resized to (has to equal '--width', if '--square' is set)",
        default=None,
        type=int,
    )
    parser.add_argument(
        "--square",
        dest="square",
        help="make the frame square (it will not be resized but cropped to the smaller edge length, if '--width' and '--height' are also set, they must have the same value)",
        action="store_true",
        default=False,
    )
    return parser
